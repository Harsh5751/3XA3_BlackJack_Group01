/**
 * GameplayLogic
 * @module GamePlayLogicZappit
*/

/**
 * @class GameplayLogicZappit class for the zappit game mode
 */
class gamePlayLogicZappit {

  /**
   * Creates an instance of gamePlayLogicZappit.
  ...
  constructor(gamePlayer, gameDealer, gameDeck) {
    ...
    */
    ...
    /**
    * @property {boolean} zapped A flag to keep trap of when a player zapped
    */
    this.zapped = false;
  }
  ...
  checkForWinner() {
    ...
    if (this.gameOver) {
      ...
      if (this.player.playerScore > 21) {
        this.playerWon = false;
      } else if (this.player.playerScore < 22 && this.dealer.dealerScore > 22) {
        this.playerWon = true;
      } else if (this.player.playerScore > this.dealer.dealerScore) {
        this.playerWon = true;
      } else if (this.player.playerScore == this.dealer.dealerScore || this.dealer.dealerScore == 22) {
        this.playerWon = null;
      } else {
        this.playerWon = false;
      }
  ...
  declareWinner() {
    ...
    if (this.playerWon == true && this.zapped == false && this.player.playerScore == 21 && this.player.playerCards.length == 2) {
      ...
      swal({
        title: "You Win This Round! Bet Win: $" + this.player.currentWager + " and No Wager Amount Lost",
        text: "Select Options Below to Play Again or Not",
        icon: "success",
        button: "Close!",
      });
  ...
  /**
   * @property {Function} reset Resets the player when zapped
   * @memberof gameplayLogicZappit
   */
  zappitReset() {
    this.player.playerCards = [];
